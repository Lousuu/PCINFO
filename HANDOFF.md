...
- CI stabilization: initial pilot-head run `29910816910` built production successfully and passed 1004 tests, but the two new Classic content guards hashed checkout-specific LF/CRLF bytes. The guards now normalize line endings before hashing the unchanged Classic content; two subsequent local full Release processes both pass 1006/1006. The final documentation-head CI run is recorded in PR #7 and the final task report.
