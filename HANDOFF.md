...
- ContentRendered 只投递一次 Render-priority 回调；该回调报告 Startup Surface Ready 并直接释放 Opacity。独立 `Task.Delay(500)` 是唯一超时释放路径，generation 与原子状态保证只有一个成功释放者。Closing/Closed 使迟到回调失效，托盘恢复、重复 Prepare、Classic 和 Motion Off 不会重新隐藏窗口。
- `StartupSequenceService` 在 Bind 仍反映瞬时 readiness，但只在进入/处于 Lock 时锁存授权。授权一旦成立，新 PollingVersion 可以更新最终 Projection 和使旧回调失效，却不能令 `CanCommit` 回落。
- Overlay 在 `PlayCommit` 后保持第二层视觉 latch。后续 Lock Snapshot 不能折叠 Group/Lock；90 ms 退出不会重新显示已隐藏 Group，并同时清除 Group、Lock、Text、Center Clip 与 minimum/compensation clocks，杜绝 COMMIT 消失和文字单独回弹。
- 首个 Reveal Snapshot 立即建立不可逆边界、终止 Projection、清空普通阶段队列并原子提交 `05 / 05 REVEAL` 的 Text、Code、颜色和五段 Track。Full/Standard/Reduced hold 为 100/80/40 ms，随后 Content、Bottom Rail、Background 和 COMMIT 在同一提交点统一 90 ms 到 0；完成后 Overlay 折叠、禁用命中并清除所有临时 Clock/Clip/Transform。
- Shell 在同一 Reveal Snapshot 后方启动：Full 为 `0/90, 20/90, 40/120, 70/90 ms`，最晚 160 ms；Standard 为 `0/70, 15/70, 30/100, 50/70 ms`，最晚 130 ms；Reduced 四目标同时 120 ms opacity-only，Classic 维持简单 120 ms。
- `SYS/BOOT.00` 使用左对齐自然宽度。Full/Standard 只有在 Measure/Arrange、Actual/Desired 尺寸有效且一致时才冻结宽高并创建一个 Clip；首次不稳定只投递一次 Render retry，第二次或阶段已前进则直接提交最终状态。Reset、Reveal、Complete、Cancel 和 Unload 使 pending generation 失效。
- 新增 10 个独立真实 WPF/服务 focused 组，每组 20/20，使自动化总数从 1857 增至 2057；920×620、1107×685、1120×720、1600×900 均覆盖。最终证据还要求 Advanced Sensors 15/15、SYSTEM REWIRE 20/20、两轮完整 Release、三类 0-warning/0-error build、Runtime XAML、package audit、PR CI、合并后 main CI 和 package workflow。
- 用户明确授权在全部门禁成功后将 PR #9 转 Ready、使用 merge commit 合并、创建 annotated `v2.0.1` 并发布唯一 framework-dependent win-x64 single-file/untrimmed `HardwareVision.exe`。不得修改 v2.0.0，不启动正式管理员 EXE；Codex 未查看截图/录屏，修复后没有新的人工录屏验收。

## HardwareVision 2.0.1 final INITIAL TRACE presentation polish

- The physical `MainWindow`, its outer root, `MainShellHost`, and the startup overlay now own the same direct `#0B0E11` first-frame surface. App calls the idempotent `PrepareFirstFrame` path before `MainWindow.Show()`, so neither Tracework nor Classic depends on a later DynamicResource lookup to cover the system-default white surface. The final release gate above supersedes this earlier static-only approach with a bounded opacity-hidden native gate; no transparent Window was added.
...
- Scope remains unchanged: no hardware scan, polling loop, DispatcherTimer, rendering loop, Advanced Sensors or SYSTEM REWIRE production change. This line records the earlier candidate stage; the final release authorization and remaining validation boundaries are defined by the section above.
...
- Baseline / scope: expanded from `e2fa91b986ed293eb24f9940796a65e269055ce4` on `feature/tracework-ui` inside existing Open/Draft/Unmerged PR #7. Dashboard `SYSTEM STATE` and CPU `PACKAGE TELEMETRY` remain the accepted pilots and were not redesigned.
- Prototype map: GPU `RENDER PIPELINE` (8/4); Memory `MEMORY TOPOLOGY` (7/5); Disk `STORAGE HEALTH` (5/7); Network `LINK TELEMETRY` (8/4); Motherboard `PLATFORM IDENTITY` (7/5); Advanced Sensors `SENSOR MATRIX` (3/9); Game Performance `CAPTURE CONTROL` (5/7); Session Report `SESSION DIAGNOSIS` (4/8); Settings `SYSTEM CONTROL` (3/9); Metric Visibility `METRIC ROUTING` (3/9).
- Shared language: CapacityField, TopologyMatrix, ControlWorkspace, TimelineField, and IdentityPlate extend the existing InstrumentField, DataRail, AnnotationRail, ChartField, MicroChart, SignalMatrix, TechnicalPanel, and `TraceworkResponsiveGrid`. Neutral surfaces remain dominant; IonViolet is identity/selection, SignalCyan is telemetry, Mint is real active/connected/success, Amber is real attention/degraded, and Coral is real failure/critical only.
- Responsive behavior: all ten expanded pages use explicit internal-width placements at Wide `>=1360` / 12 columns, Standard `>=960` / 8, Compact `>=680` / 4, and Narrow `<680` / 1. Compact and Narrow stack primary/secondary regions without horizontal scrolling.
- Binding and Classic protection: selectors, commands, two-way settings, chart windows, tooltips, visibility, virtualization, nested report scrolling, TARGET PROCESS dimensions, capture lifecycle, session/report formats, and settings persistence remain. All ten corresponding Classic layouts are protected by normalized SHA-256 tests.
- Architecture compatibility: hardware collection, polling, sensor/GPU history, PresentMon, recorder, single PageHost/CurrentPage binding, page-VM cache, `SetPageActive`, `LastSelectedPage`, SYSTEM REWIRE, and FLOW RELAY state/commit semantics are unchanged. Every expanded page has at most one Primary and one Secondary navigation role; item/data rows have none.
- Automated validation: the expanded suite totals `1136` cases, above the `1006` baseline, including runtime XAML, five-width Measure/Arrange, bindings, semantics, and Classic protection. One intervening run reproduced the documented transient `Report accuracy 01` timestamp case (`1135/1/1136`, expected 10 / got 5); report code was unchanged, and both subsequent independent full Release processes passed `1136/0/1136`.
- Validation boundary: manual visual validation, screenshot analysis, real-DPI validation, and formal administrator EXE launch were not performed.
- CI / final head: the final documentation-head CI result and exact full SHA are recorded in Draft PR #7 and the final task report; a tracked commit cannot contain its own hash.
...
- Baseline / scope: based on `10e997944e7cd516ed669d15b27a88fba7c7a272` on `feature/tracework-ui`, within existing Open/Draft/Unmerged PR #7. The versioned navigation state machine, Relay commit point, fast-navigation policy, SYSTEM REWIRE priority, single PageHost/current-page binding, page lifecycle, and hardware/business services were preserved.
- Signal and title: the single SignalRail overlay now contains a real-button-center `RouteSegment`, `RoutePulse`, Full-only `PulseTrail`, and committed-target `ArrivalLock`. Telemetry is a clipped overlapping Source/Target dual track: the origin remains distinct until commit, the committed target alone becomes the polite live region, and the target subtitle enters during the last `70ms` of Settle.
- Relay and page: the sole RelayBand template now has machine body, `2px` leading edge, `1px` trailing edge, two traces, two nodes, Full-only internal pulse, short route code, and committed-snapshot `CenterLock`. The band crosses center at the unchanged commit time with cubic EaseInOut. The one PageHost reveals its existing motion surface through a directional `RectangleGeometry` clip, opacity and a small translate; no second host, page copy, scale, blur, shader, or layout-property animation was introduced.
- Profiles: Full uses `150ms / 0.74 / 10px` page reveal/opacity/offset and `28/72ms`, `0.68/0.58`, `8/12px` Primary/Secondary settle values. Standard uses `118ms / 0.80 / 7px` and `18/48ms`, `0.78/0.70`, `6/8px`. Reduced is opacity-only with no spatial route, band internals, clip reveal, or module stagger. Off commits directly without visual clocks.
- Cleanup / isolation: completion, cancellation, rapid replacement, resize, unload, and SYSTEM REWIRE takeover clear opacity/translate/clip clocks and stale versions. LIVE/polling/time/hardware text, item rows, `SetPageActive`, `LastSelectedPage`, page ViewModels, Polling, PresentMon, recorder, and GPU history remain outside the visual transition.
- Automated validation: clean Release and Debug application builds pass with `0 warning / 0 error`; two independent final full Release test processes pass with `891 passed, 0 failed, 891 total`, identical and above the `791` baseline; `git diff --check` passes. One preceding run reproduced the existing transient `Report accuracy 01` timestamp case at `890/1/891` (`expected 10, got 5`); no report/session/PresentMon source was changed, and both subsequent complete runs passed. The successful final documentation-head CI run and exact final SHA are recorded in PR #7 and the final task report.
- Validation boundary: manual visual validation was not performed; screenshots were not inspected; the formal administrator EXE was not launched; real-DPI validation was not performed. Automated coverage does not replace final manual visual acceptance.
...
## FLOW RELAY navigation transition (completed architecture baseline)
...
- Validation: clean Release and Debug application builds passed with `0 warning / 0 error`. Two independent full Release test processes both passed with `791 passed, 0 failed, 791 total` (82 above the 709 baseline). Initial CI run `29900694241` built production successfully but its temporary-artifacts test EXE exposed a source-root lookup defect in four new static-test fixtures (`747/44/791`); no product test failed. The fixtures now search the workflow working directory before their external binary directory, and an exact repository-external artifacts reproduction passed `791/0/791`. `git diff --check` passed. Manual visual validation, screenshot analysis, formal administrator EXE launch, and real-DPI validation were not performed.
...
## TRACEWORK UI layout stabilization (historical stage record)
...
- Memory: module metrics now use one shared item template and `AdaptiveUniformGrid`. Visible items compact in source order; columns are 3 at `>=1080px`, 2 at `680-1079px`, and 1 below `680px`, with `12px` gaps. Cards are at least `92px` high and retain full value tooltips. Existing raw `FormFactor` / `MemoryType` values remain unchanged because no verified local enum mapping was found.
- Game TARGET PROCESS: the existing Tracework panel now presents header/status, subtitle, shared 5*:9* label/input columns with a `28px` gap, and a wrapping four-button row. TextBox/ComboBox and target buttons are `64px` high; target buttons are at least `172px` wide. The duplicate status text after the buttons was removed while the formal right-top `StatusText` binding and all commands/data bindings remain.
- Shared layout: all 12 Tracework page roots use the shared stretch/min-size contract. Ten long pages use `TraceworkPageScrollViewerStyle` with disabled horizontal scrolling and `0,0,12,24` safe padding; Advanced Sensors and Metric Visibility use the shared fixed-grid content host. SignalRail items use ellipsis/no-wrap plus full-title Tooltip and `AutomationProperties.Name`.
- Automated validation: clean Release and Debug application builds passed with `0 warning / 0 error`. After the deterministic test-fixture update, two independent custom-runner processes both passed with `709 passed, 0 failed, 709 total` (49 tests above the 660 baseline). An earlier unrelated local run exposed the pre-existing transient `Report accuracy 01` failure (`708/1/709`); report tests and report business logic were not changed. `git diff --check` passed. Manual visual validation, screenshot analysis, formal administrator EXE launch, and real-DPI validation were not performed.
- Historical GitHub note: at that layout-stage point, FLOW RELAY was not yet implemented. That statement is superseded by the completed architecture and final visual amplification sections above. The initial layout commit `462901647c3c116681a434faac876a327f84a220` failed CI run `29896515742` only because the page-level responsive test inferred internal content width from an outer Window; test-fix commit `ee8075ceb80fe435793c578e990cc36275daebe1` passed in run `29897148260`.

## TRACEWORK UI Stage 5 handoff update (historical stage record)
...
## TRACEWORK UI bugfix stabilization (historical stage record)

- Historical scope at that point: four targeted fixes on `feature/tracework-ui` for Tracework page fade replay/visibility, game-session report nested scrolling, shared GPU history lifetime, and PageHost background gaps. FLOW RELAY was not implemented during that historical pass; it is complete now as recorded above.
...
## 14. HardwareVision 2.0.2 candidate DPI placement correction and review

- Draft PR #10 originally computed `CenterScreen` from `GetMonitorInfo` physical pixels and assigned those values to WPF `Window.Left` / `Top`, which are DIP. At 125%, 150%, 175%, and 200% this interpreted physical coordinates as logical coordinates and shifted the cold-start window toward the lower-right.
- `HardwareVision.Interop.WindowPlacementInterop` now captures the cursor-selected monitor handle, physical work area, effective X/Y DPI, and one immutable physical target rectangle before off-screen staging. It converts the 1120×720 DIP window to physical size with `DIP × DPI / 96`, centers entirely in physical coordinates, stages the existing HWND outside the physical virtual desktop, and restores it through `SetWindowPos`. Later cursor movement cannot select a different monitor for release.
- The existing First Frame Gate remains `Dormant -> NativePrepared -> ShownHidden -> FirstRenderCommitted -> NativeCompositionFlushed -> FinalPlacementCommitted -> Released`; its two Render boundaries, both `DwmFlush` points, 500 ms generation-guarded fail-open, opacity cover, close cancellation, and tray one-shot behavior are unchanged. Classic and Motion Off still leave the gate immediately; no Window, Splash, title-bar system, timer, render-loop subscription, polling path, hardware read, or animation phase was added.
- Ten DPI/multi-monitor cases repeat 20 times (`200/0/200`) and cover 100/125/150/175/200%, negative-left, right-side, lower, mixed-DPI, and cursor-after-capture scenarios with a two-physical-pixel centering tolerance. The pre-existing v2.0.2 visual suite remains `240/0/240`; both focused stderr files are empty.
- Review confirmed one `MainWindow`, one `MainShellHost`, one `PageHost`, and one App-owned `StartupSequenceService`. Startup subscriptions have terminal unsubscription/cleanup; the polling path remains single-flight with a coalesced schedule wake-up; pages remain lazily created; startup `UpdateLayout` calls are bounded phase commits rather than continuous work. The startup snapshot LINQ is bounded to small fixed milestone/slot sets. No additional production performance edit was justified, so no speculative timer, scan, collection rewrite, or service refactor was introduced.
