...
HardwareVision 是一款面向 Windows 的轻量硬件与游戏性能监控工具，使用 WPF 和 .NET 8 构建。当前版本为 v0.1.7。
...
- 自动生成逐帧 CSV、限制事件 CSV、硬件时间线 CSV 与 JSON 摘要；异常退出时保留并恢复 partial 数据。
...
- 完整会话按 `yyyy-MM` 月份目录保存逐帧 `.csv`、`.performance-limits.csv`、`.hardware-timeline.csv` 和 `.summary.json`。
- 写入中的逐帧/时间线文件使用 partial 后缀；下次启动时，有数据的残留文件恢复为 incomplete 文件，空文件安全删除。
...
测试仍使用项目自带的控制台运行器；v0.1.7 共 216 项，覆盖 PresentMon 解析/状态机、进程选择、性能与会话隔离、能耗积分、限制事件、磁盘可靠性、硬件时间线、静态报告、下采样、旧记录兼容、缓存与自动记录端到端链路。
