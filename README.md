...
从 [HardwareVision v2.0.0 Release](https://github.com/Lousuu/PCINFO/releases/tag/v2.0.0) 下载唯一的发布资产：
...
测试仍使用项目自带的控制台运行器。v2.0.0 的发布验证为 `1366 passed, 0 failed, 1366 total`，覆盖既有硬件/PresentMon/会话链路，以及 TRACEWORK 双主题、响应式布局、FLOW RELAY、SYSTEM REWIRE、INITIAL TRACE、启动生命周期、发布打包与资产约束。报告时间测试使用固定单调时间源，消除了依赖系统时钟分辨率的偶发失败。
