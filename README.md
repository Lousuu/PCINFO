...
- 会话报告会流式重新校验普通/GZip 历史 CSV，主要区域展示 8 帧窗口平均帧时间倒数的稳健最大 FPS；原始单帧最大值仅保留在质量诊断区。
...
测试仍使用项目自带的控制台运行器。公开 v0.1.7 的发布基线为 216 项；当前开发基线为 357 项，覆盖 PresentMon 解析/状态机、warm-up 与稳定阶段稳健过滤、每链 SwapChain 状态、严格时间戳、辅助字段清洗、稳健最大 FPS、主 SwapChain 选择、进程选择、性能与会话隔离、能耗积分、限制事件、硬件热插拔刷新、磁盘可靠性、GZip 会话存储、summary schema v4、静态报告、下采样、旧记录兼容、缓存与自动记录端到端链路。
