...
测试仍使用项目自带的控制台运行器。公开 v0.1.7 的发布基线为 216 项；当前开发分支为 `449 passed, 0 failed, 449 total`，覆盖 PresentMon 解析/状态机、warm-up 与稳定阶段异常过滤、每链 SwapChain 状态、严格时间戳、辅助字段清洗、最大 FPS、会话历史分页与并发取消、短会话曲线、主 SwapChain 选择、性能与会话隔离、异步取消生命周期、能耗积分、限制事件、硬件热插拔刷新、磁盘可靠性与外接桥接身份合并、GZip 会话存储、summary schema v4、历史会话报告、旧记录兼容、缓存与自动记录端到端链路。
