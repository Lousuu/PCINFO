...
测试仍使用项目自带的控制台运行器。v2.0.1 的发布验证为 `1432 passed, 0 failed, 1432 total`，新增真实 WPF 模板、Measure/Arrange、AnimationClock、首帧可见性、投影门与冷模板重放覆盖，同时保留既有硬件/PresentMon/会话链路。报告时间测试使用固定单调时间源，消除了依赖系统时钟分辨率的偶发失败。
