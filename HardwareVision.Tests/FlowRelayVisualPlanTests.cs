...
internal static class FlowRelayVisualPlanTests
{
    ...
    [
        ...
        ("Flow visual plan 16 decorative relay keeps original timing", DecorativeRelayKeepsOriginalTiming)
    ];
    ...
    private static void DecorativeRelayKeepsOriginalTiming()
    {
        TestSupport.Equal(TimeSpan.FromMilliseconds(190), Plan(MotionLevel.Full).CommitTime, "Full relay");
        TestSupport.Equal(TimeSpan.FromMilliseconds(140), Plan(MotionLevel.Standard).CommitTime, "Standard relay");
        TestSupport.Equal(TimeSpan.FromMilliseconds(50), Plan(MotionLevel.Reduced).CommitTime, "Reduced relay");
    }
}
