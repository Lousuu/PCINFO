    ...
    private void AddSample(GameFrameSample sample, CaptureDiagnostics diagnostics)
    {
        ...
        if (diagnostics.AcceptedSampleCount == 1)
        {
            AppLogger.LogKeyEvent(
                ...
                + $"; fpsSource={sample.PrimaryFpsSource}"
                + $"; applicationFps={FormatLog(sample.ApplicationFps)}"
                + $"; presentedFps={FormatLog(sample.PresentedFps)}"
                + $"; displayedFps={FormatLog(sample.DisplayedFps)}"
